...
// ПЕРЕВІРКА: Якщо файлу немає (сервер прокинувся) АБО він старіший за 2 години (7200 сек) -> генеруємо
            ...
            while ($reader->read()) {
                if ($reader->nodeType == XMLReader::ELEMENT && $reader->name == 'offer') {
                    ...
                    if ($offer_node) {
                        ...
                        fwrite($out, '    <offer id="' . $offer_id . '" available="' . $available . '"></offer>' . "\n");
                        unset($offer_node);
                    }
                }
            }
...
// ВІДДАЄМО ГОТОВИЙ ФАЙЛ ПРЯМО В БРАУЗЕР (АБО ПРОМУ)
